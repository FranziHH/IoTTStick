...
def merge_bin_action(source, target, env):
    ...
    output_file = os.path.join(output_dir, f"{version}_{release_name}_full.bin")
    ...
    boot_app0 = os.path.join(project_dir, "bin", "boot_app0.bin")
    bootloader = os.path.join(project_dir, "bin", "m5stick_plus2.bootloader.bin")
    partitions = os.path.join(build_dir, "partitions.bin")
    firmware = os.path.join(build_dir, "firmware.bin")
    # NEU: Das Filesystem-Image
    filesystem = os.path.join(build_dir, "spiffs.bin")

    if not all(os.path.isfile(f) for f in [partitions, firmware]):
        print(f"--- [MERGE] Fehler: Binärdateien in {build_dir} nicht gefunden! ---")
        return

    # 3. Das esptool Kommando zusammenbauen
    cmd = [
        f'"{python_path}"', "-m", "esptool", "--chip", "esp32s3", "merge-bin",
        "-o", f"\"{output_file}\"",
        "0x1000", f"\"{bootloader}\"",
        "0x8000", f"\"{partitions}\"",
        "0xE000", f"\"{boot_app0}\"",
    ...
    if os.path.isfile(filesystem):
        ...
        cmd.extend(["0x670000", f"\"{filesystem}\""])
    else:
        print(f"--- [MERGE] HINWEIS: Keine spiffs.bin gefunden. Erzeuge Image ohne Presets. ---")

    print(f"--- [MERGE] Schutz-Modus: Übernehme Bootloader-Header für build_output/merge/{version}_{release_name}_full.bin ---")
